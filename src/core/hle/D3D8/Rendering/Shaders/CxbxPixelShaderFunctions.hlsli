...
//
// Shared by CxbxPixelShaderTemplate.hlsl (compiled PS), FixedFunctionPixelShader.hlsl,
// and RegisterCombinerInterpreter.hlsl (RC interpreter ubershader).
//
...
float CalculateFogFactor(CXBX_STEERING_INT fogEnable, CXBX_STEERING_INT fogTableMode, float fogDensity, float fogStart, float fogEnd, float fogDepth)
{
	...
	if (fogEnable != 0) {
		if (fogTableMode == 0) // NONE (vertex fog passthrough)
			...
			fogFactor = 1 / exp(fogDepth * fogDensity);
		else if (fogTableMode == 2) // EXP2
			fogFactor = 1 / exp(pow(fogDepth * fogDensity, 2));
