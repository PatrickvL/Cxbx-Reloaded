...
)
{
	...
	// Diagnostic: Log KeDelayExecutionThread calls
	static int diag_delay_count = 0;
	if (diag_delay_count < 50) {
		fprintf(stderr, "[KEDELAY] tid=0x%04X interval=%lld alertable=%d\n",
			GetCurrentThreadId(),
			(Interval ? Interval->QuadPart : 0LL),
			Alertable);
		diag_delay_count++;
	}
	if (Interval && Interval->QuadPart == LLONG_MIN) {
		fprintf(stderr, "[KEDELAY-INF] tid=0x%04X infinite wait\n", GetCurrentThreadId());
	}
...
)
{
	...
	// DIAG: Track KeWaitForSingleObject calls to identify loading hang
	{
		...
		// Log first 50 calls unconditionally + any infinite waits
		if (s_keWaitCount <= 50 || (!Timeout)) {
			...
			WaitStatus = WaitApc<false>([Object](PKTHREAD Thread) -> std::optional<ntstatus_xt> {
				...
				if (satisfiable) {
					...
					if (satisfiable) {
						...
						KiWaitSatisfyAny(ObjectMutant, Thread);
						KiRemoveWaitBlocks(Thread);
						// Mark thread as Ready BEFORE releasing KiWaitListLock
						// to prevent KiUnwaitThread from a racing KiTimerExpiration
						// from double-removing the already-removed WaitBlocks.
						Thread->State = Ready;
						KiWaitListUnlock();
						KiCancelThreadTimer(Thread);
						return std::make_optional<ntstatus_xt>(Thread->WaitStatus);
