...
### 43. Palette Texture Expansion on GPU via Compute Shader
- [x] Fixed
- **Files**: `Backend_D3D11.cpp`, `Backend_D3D11.h`, `HostResourceCreate.cpp`
- **Description**: Xbox P8 (8-bit paletted) textures are expanded to 32-bit RGBA on CPU via `ConvertD3DTextureToARGBBuffer()`, which first calls `EmuUnswizzleBox()` (P8 is a swizzled format) into a temporary buffer, then iterates per-row with a palette lookup function. A compute shader (cs_5_0) combines both passes: it reads raw swizzled P8 bytes via ByteAddressBuffer, performs Morton/Z-order decode (same as unswizzle CS), uses the byte as an index into a 256-entry palette uploaded as a second ByteAddressBuffer, and writes the RGBA result directly to the target texture through a R32_UINT UAV. The palette ARGB→RGBA byte swap is performed in the CS. Single-mip 2D P8 textures are created as `DXGI_FORMAT_R8G8B8A8_UNORM` with DEFAULT+UAV instead of `B8G8R8A8_UNORM` DYNAMIC. CPU fallback via the existing `ConvertD3DTextureToARGBBuffer` + `UpdateSubresource` path is retained. Missing palette case (e.g. DRIV3R) zero-fills via `UpdateSubresource` since the texture is DEFAULT and cannot be mapped.
- **Resolution**: Implemented. The palette buffer (1024 bytes, 256 ARGB entries) is a persistent DYNAMIC ByteAddressBuffer uploaded each dispatch. The CS reuses the existing unswizzle staging buffer for P8 source data. Bounds checking covers both width and height to handle sub-8-pixel textures. P8 is always swizzled on Xbox (no LIN_P8 format exists). CPU writes to P8 textures trigger full texture re-creation via the resource hash/cache system, so the CS runs again on modified data.
- **Impact**: Performance — palette expansion + unswizzle combined into a single GPU dispatch

### 44. Vertex Format Patching on GPU via Compute Shader
- [x] Fixed
- **Files**: `Backend_D3D11.cpp`, `Backend_D3D11.h`, `XbVertexBuffer.cpp`
- **Description**: Xbox vertex formats that have no native DXGI equivalent are converted per-vertex on CPU in `CxbxVertexBufferConverter::ConvertStream()`. Despite D3D11 eliminating many format cases (NORMSHORT1/2/4, PBYTE1/2/4, SHORT1 now use native DXGI formats), several conversions remain CPU-bound: `NORMPACKED3` (11/11/10-bit unpack → 3 floats), `NORMSHORT3` (3 shorts → SHORT4N with W=32767), `SHORT3` (3 shorts → SHORT4 with W=1), `PBYTE3` (3 bytes → UBYTE4N with A=255), `FLOAT2H` (3 floats → 4 floats with Z=0 insert), and `D3DCOLOR` (BGRA→RGBA byte swap). A compute shader encodes the per-vertex element layout (offsets, types, sizes) in a constant buffer and processes all vertices in parallel. Source vertex data is uploaded to a ByteAddressBuffer SRV; output is written to a DEFAULT vertex buffer via a typed UAV. The CS is dispatched with one thread per vertex, each iterating the element descriptor array.
- **Resolution**: Implemented. The CS (cs_5_0, 64 threads/group) reads source vertices from a ByteAddressBuffer using unaligned-read helpers (shift from aligned Load), and writes to a `RWBuffer<uint>` (DXGI_FORMAT_R32_UINT UAV) on a DEFAULT+VB+UAV buffer. Up to 16 element descriptors are packed into a 272-byte constant buffer (header + 16×uint4). Seven conversion types are supported: COPY (N dwords), NORMSHORT3, NORMPACKED3, SHORT3, PBYTE3, FLOAT2H, D3DCOLOR, plus NONE (skip). The CS path is only used when all host element sizes and the host stride are multiples of 4 (required for typed UAV writes); vertex formats with sub-4-byte host elements (NORMSHORT1, PBYTE1/2, SHORT1) fall back to CPU. DrawUP (stream-zero) also falls back to CPU. The output buffer is cached in the existing `CxbxPatchedStream` hash-based system and released via `Clear()→Release()` on eviction.
- **Impact**: Performance — vertex format patching offloaded to GPU, eliminating per-vertex CPU conversion loops
